...
export function StepProvider({ children }) {
  ...
  const runEvaluation = useCallback(async (nextAnswers, nextWorkflow = workflow, nextCatalog = catalog) => {
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      ...
      const resumeQuestion = savedQuestion
  ...
  const currentQuestion = useMemo(() => {
    ...
    return questions.find(question => question.id === currentQuestionId) || evaluation?.nextQuestion || questions[0] || null;
  }, [currentQuestionId, evaluation, workflow]);

  const currentIssue = useMemo(() => evaluation?.issues?.find(issue => issue.path === `answers.${currentQuestion?.id}`), [evaluation, currentQuestion]);
  ...
  const continueFlow = async () => {
    ...
    try {
      ...
      const issue = result?.issues?.find(item => item.path === `answers.${currentQuestion.id}`) || result?.issues?.[0];
      if (issue) {
        const issueQuestionId = issue.path?.replace("answers.", "");
        if (result.visibleQuestions?.some(question => question.id === issueQuestionId)) setCurrentQuestionId(issueQuestionId);
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError(cause.issues?.map(issue => issue.message).join(" ") || cause.message || "No se pudo generar el bundle.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const value = {
    phase, catalog, workflow, tutorial, tutorialIndex, answers, evaluation,
    currentQuestion, currentIssue, bundle, loading, error,
    canContinue: currentQuestion ? hasAnswer(currentQuestion, answers[currentQuestion.id]) : false,
    initialize, updateAnswer, continueFlow, goBack, goToQuestion,
    skipTutorial, continueTutorial, generate, reset,
  };
