...
const initialLogs = ['[HuascarEngine] Sistema inicializado.', '[HuascarEngine] Esperando instrucciones...'];

const limitLogs = (logs: string[]) => logs.slice(-500);

export function useAgentExecution(role: string, task: string, agentConfig: AgentConfig | null, onComplete: () => void) {
  ...
  const execute = async () => {
    ...
    addLogs(`[HuascarEngine] Desplegando agente con rol: ${role}...`, `[HOOK] Validando tarea: "${task}"...`);
