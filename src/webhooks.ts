...
const WEBHOOK_URLS = (process.env.WEBHOOK_URLS || '').split(',').map(u => u.trim()).filter(Boolean);
...
export async function emitWebhook(event: WebhookEvent): Promise<void> {
  ...
  for (const url of WEBHOOK_URLS) {
    try {
      ...
      console.warn(`[Webhook] Failed to deliver to ${url}: ${err instanceof Error ? err.message : 'unknown'}`);
    }
  }
}
